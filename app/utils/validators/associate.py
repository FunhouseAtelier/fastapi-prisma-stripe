# app/utils/validators/associate.py
...
from app.schemas.associate import (
    BusinessNameStr,
    CityStr,
    EmailAddrStr,
    FullNameStr,
...
class AssociateFormSchema(BaseModel):
    name: FullNameStr
...
#
async def validate_associate_form(form_name: str, request: Request) -> dict:
    form = request.state.form
    try:
        if form_name == "new":
            parsed = AssociateFormSchema(**form)
            return {
                "success": {
                    "associate": parsed.model_dump(exclude_none=True)
                }
            }
        elif form_name == "edit":
            parsed = AssociateFormSchema.with_unset(form)
            return {"success": {"associate": parsed.model_dump()}}
